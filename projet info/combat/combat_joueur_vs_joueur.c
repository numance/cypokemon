...
int choixactionj2(Poke* joueur1, Poke* joueur2, int tour) { //permet de se deplacer dans le menu pour choisir notre choix
    int choix = 0;  //l'option sélectionnée à la base
    int touche;
    int nbratt=4;
    while (1) { //boucle infini tant que touche diiferent de entrée
        ...
        for (int i = 0; i < nbratt; i++) {
            if (i == choix) {
                printf("    > %s <\n", joueur2->attaque[i]->nom); //option selectionnés
            } else {
                printf("      %s\n", joueur2->attaque[i]->nom);  // Autres options
            }
        }
...
int tourn(Poke* joueur1, Poke* joueur2, int tour) { // si pokemon ko retourne 1 et met fin a la boucle infini
    ...
    int actionj2=choixactionj2(joueur1, joueur2, tour);
...
int main() {
    ...
    joueur1.type=0;  // type correspondant ici normal
