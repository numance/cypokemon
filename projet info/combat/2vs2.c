...
void affichagePokepv(Poke* j1[2], Poke* j2[2], int tour, Actif* actif) {
    system("clear");
...
int choixattj1(Poke* joueur1[2], Poke* joueur2[2], int tour, Actif* actif) { //permet de se deplacer dans le menu pour choisir notre choix
...
int choixchangementpokej1(Poke* joueur1[2], Poke* joueur2[2], int tour, Actif* actif) { //permet de se deplacer dans le menu pour choisir notre choix
    int choix = 0;  //l'option sélectionnée à la base
    int touche;
    int nbrpoke=1;
    while (1) { //boucle infini tant que touche diiferent de entrée
        ...
        if (actif->actif1==1) {
            printf("    > %s  %d/%d <\n", joueur1[actif->actif1-1]->name, joueur1[actif->actif1-1]->pvactuel, joueur1[actif->actif1-1]->pvmax); //option selectionnés
        } else {
            printf("    > %s  %d/%d <\n", joueur1[actif->actif1+1]->name, joueur1[actif->actif1+1]->pvactuel, joueur1[actif->actif1+1]->pvmax); //option selectionnés
        }
...
int choixtypeactionj1(Poke* joueur1[2], Poke* joueur2[2], int tour, Actif* actif) { //permet de se deplacer dans le menu pour choisir notre choix
    ...
    while (1) { //boucle infini tant que touche diiferent de entrée
        affichagePokepv(joueur1, joueur2, tour, actif);
        printf("  choix du type d'action 1\n\n");
...
int choixactionj1(Poke* joueur1[2], Poke* joueur2[2], int tour, Actif* actif) {
    int choix2;
    int choix1;
    if(joueur1[0]->pvactuel!=0 && joueur1[1]->pvactuel!=0) {
        choix1=choixtypeactionj1(joueur1, joueur2, tour, actif);
        if(choix1==1) {
            choix2=choixchangementpokej1(joueur1, joueur2, tour, actif);
         } else {
...
int choixattj2(Poke* joueur1[2], Poke* joueur2[2], int tour, Actif* actif) { //permet de se deplacer dans le menu pour choisir notre choix
    int choix = 0;  //l'option sélectionnée à la base
    int touche;
    int nbratt=4;
    while (1) { //boucle infini tant que touche diiferent de entrée
...
int choixchangementpokej2(Poke* joueur1[2], Poke* joueur2[2], int tour, Actif* actif) { //permet de se deplacer dans le menu pour choisir notre choix
    int choix = 0;  //l'option sélectionnée à la base
    int touche;
    int nbrpoke=1;
    while (1) { //boucle infini tant que touche diiferent de entrée
        ...
        if (actif->actif2==1) {
            printf("    > %s  %d/%d <\n", joueur2[actif->actif2-1]->name, joueur2[actif->actif2-1]->pvactuel, joueur2[actif->actif2-1]->pvmax); //option selectionnés
        } else {
            printf("    > %s  %d/%d <\n", joueur2[actif->actif2+1]->name, joueur2[actif->actif2+1]->pvactuel, joueur2[actif->actif2+1]->pvmax); //option selectionnés
        }
...
int choixtypeactionj2(Poke* joueur1[2], Poke* joueur2[2], int tour, Actif* actif) { //permet de se deplacer dans le menu pour choisir notre choix
    int choix = 0;  //l'option sélectionnée à la base
    int touche;
    int nbrtype=2;
    char* type[2]={"attaque","changer de pokemon"};
...
int choixactionj2(Poke* joueur1[2], Poke* joueur2[2], int tour, Actif* actif) {
    int choix2;
    int choix1;
    if(joueur2[0]->pvactuel!=0 && joueur2[1]->pvactuel!=0) {
        choix1=choixtypeactionj2(joueur1, joueur2, tour, actif);
        if(choix1==1) {
            choixchangementpokej2(joueur1, joueur2, tour, actif);
         } else {
...
int attfonction0j2(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj2, int esquive1, int reussite2, Actif* actif) { // pour attaque du j2
    affichagePokepv(joueur1, joueur2, tour, actif);
...
int attfonction0j1(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj1, int esquive2, int reussite1, Actif* actif) { // pour attaque du j1
    affichagePokepv(joueur1, joueur2, tour, actif);
...
int attfonction1j1(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj1, int reussite1, Actif* actif) { // pour regeneration du joueur 1
    affichagePokepv(joueur1, joueur2, tour, actif);
        printf("%s utilise %s\n", joueur1[actif->actif1]->name, joueur1[actif->actif1]->attaque[actionj1]->nom);
        sleep(4);
        if(joueur1[actif->actif1]->attaque[actionj1]->reussite<reussite1) {
...
int attfonction1j2(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj2, int reussite2, Actif* actif) { // pour regeneration du j 2
    affichagePokepv(joueur1, joueur2, tour, actif);
        printf("%s utilise %s\n", joueur2[actif->actif2]->name, joueur2[actif->actif2]->attaque[actionj2]->nom);
        sleep(4);
...
int attfonction2j2(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj2, int reussite2, Actif* actif) { // pour augm att du j2
    affichagePokepv(joueur1, joueur2, tour, actif);
        printf("%s utilise %s\n", joueur2[actif->actif2]->name, joueur2[actif->actif2]->attaque[actionj2]->nom);
        sleep(4);
        if(joueur2[actif->actif2]->attaque[actionj2]->reussite<reussite2) {
...
int attfonction2j1(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj1, int reussite1, Actif* actif) { // pour augm att du j1
    affichagePokepv(joueur1, joueur2, tour, actif);
        printf("%s utilise %s\n", joueur1[actif->actif1]->name, joueur1[actif->actif1]->attaque[actionj1]->nom);
        sleep(4);
        if(joueur1[actif->actif1]->attaque[actionj1]->reussite<reussite1) {
...
int attfonction3j2(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj2, int esquive1, int reussite2, Actif* actif) { // pour baisse att du j1
    affichagePokepv(joueur1, joueur2, tour, actif);
        printf("%s utilise %s\n", joueur2[actif->actif2]->name, joueur2[actif->actif2]->attaque[actionj2]->nom);
        sleep(4);
        if(joueur2[actif->actif2]->attaque[actionj2]->reussite<reussite2) {
...
int attfonction3j1(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj1, int esquive2, int reussite1, Actif* actif) { // pour baisse att du j2
    affichagePokepv(joueur1, joueur2, tour, actif);
        printf("%s utilise %s\n", joueur1[actif->actif1]->name, joueur1[actif->actif1]->attaque[actionj1]->nom);
        sleep(4);
        if(joueur1[actif->actif1]->attaque[actionj1]->reussite<reussite1) {
...
int attfonction4j2(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj2, int reussite2, Actif* actif) { // pour augm def du j2
    affichagePokepv(joueur1, joueur2, tour, actif);
        printf("%s utilise %s\n", joueur2[actif->actif2]->name, joueur2[actif->actif2]->attaque[actionj2]->nom);
        sleep(4);
        if(joueur2[actif->actif2]->attaque[actionj2]->reussite<reussite2) {
...
int attfonction4j1(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj1, int reussite1, Actif* actif) { // pour augm def du j1
    affichagePokepv(joueur1, joueur2, tour, actif);
        printf("%s utilise %s\n", joueur1[actif->actif1]->name, joueur1[actif->actif1]->attaque[actionj1]->nom);
        sleep(4);
        if(joueur1[actif->actif1]->attaque[actionj1]->reussite<reussite1) {
...
int attfonction5j2(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj2, int esquive1, int reussite2, Actif* actif) { // pour baisse def du j1
    affichagePokepv(joueur1, joueur2, tour, actif);
        printf("%s utilise %s\n", joueur2[actif->actif2]->name, joueur2[actif->actif2]->attaque[actionj2]->nom);
        sleep(4);
        if(joueur2[actif->actif2]->attaque[actionj2]->reussite<reussite2) {
...
int attfonction5j1(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj1, int esquive2, int reussite1, Actif* actif) { // pour baisse def du j2
    affichagePokepv(joueur1, joueur2, tour, actif);
        printf("%s utilise %s\n", joueur1[actif->actif1]->name, joueur1[actif->actif1]->attaque[actionj1]->nom);
        sleep(4);
        if(joueur1[actif->actif1]->attaque[actionj1]->reussite<reussite1) {
...
int changementcombatj2(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj2, Actif* actif) { // pour augm att du j2
    affichagePokepv(joueur1, joueur2, tour, actif);
    if(actif->actif2==0) {
        printf("le joueur 2 change de pokemon \n%s rentre dans sa pokeball\n%s rentre dans le combat\na", joueur2[actif->actif2]->name, joueur2[1]->name);
        sleep(4);
        actif->actif2+=1;
        affichagePokepv(joueur1, joueur2, tour, actif);
    } else {
       printf("le joueur 2 change de pokemon \n%s rentre dans sa pokeball\n%s rentre dans le combat\na", joueur2[actif->actif2]->name, joueur2[0]->name);
       sleep(4);
       actif->actif2-=1;
       affichagePokepv(joueur1, joueur2, tour, actif);
    }    
...
int changementcombatj1(Poke* joueur1[2], Poke* joueur2[2], int tour, int actionj1, Actif* actif) { // pour augm att du j2
    affichagePokepv(joueur1, joueur2, tour, actif);
    if(actif->actif1==0) {
        printf("le joueur 1 change de pokemon \n%s rentre dans sa pokeball\n%s rentre dans le combat\na", joueur1[actif->actif1]->name, joueur1[1]->name);
        sleep(4);
        actif->actif1+=1;
        affichagePokepv(joueur1, joueur2, tour, actif);
    } else {
       printf("le joueur 1 change de pokemon \n%s rentre dans sa pokeball\n%s rentre dans le combat\na", joueur1[actif->actif1]->name, joueur1[0]->name);
       sleep(4);
       actif->actif1-=1;
       affichagePokepv(joueur1, joueur2, tour, actif);
    }    
...
int phaseattaque(Poke* joueur1[2], Poke* joueur2[2], int actionj1, int actionj2, int tour, Actif* actif) {
    ...
    if(actionj1==4 && actionj2==4) {
        ...
        if(joueur1[0]->pvactuel==0 && joueur1[1]->pvactuel==0) {
            ...
            sleep(2);
            if(actif->actif1==0) {
                printf("%s rentre dans le combat pour le remplacer", joueur1[actif->actif1+1]->name);
                sleep(2);
                actif->actif1+=1;
                affichagePokepv(joueur1, joueur2, tour, actif);
                return 0;
            } else {
                printf("%s rentre dans le combat pour le remplacer", joueur1[actif->actif1-1]->name); // a voir
                sleep(2);
                actif->actif1-=1;
                affichagePokepv(joueur1, joueur2, tour, actif);
                return 0;
        ...
        if(joueur2[0]->pvactuel==0 && joueur2[1]->pvactuel==0) {
            ...
            sleep(2);
            if(actif->actif2==0) {
                printf("%s rentre dans le combat pour le remplacer", joueur2[actif->actif2+1]->name);
                sleep(2);
                actif->actif2+=1;
                affichagePokepv(joueur1, joueur2, tour, actif);
                return 0;
            } else {
                printf("%s rentre dans le combat pour le remplacer", joueur2[actif->actif2-1]->name); // a voir
                sleep(2);
                actif->actif2-=1;
                affichagePokepv(joueur1, joueur2, tour, actif);
                return 0;
           ...
           if(joueur1[actif->actif1]->attaque[actionj1]->fonction==0) { // joueur 1 attaque
            ...
            if(joueur1[0]->pvactuel==0 && joueur1[1]->pvactuel==0) {
                ...
                if(actif->actif1==0) {
                    ...
                    printf("%s rentre dans le combat pour le remplacer", joueur1[actif->actif1-1]->name); // a vooir
                    sleep(2);
                    actif->actif1-=1;
                    affichagePokepv(joueur1, joueur2, tour, actif);
                    return 0;
...
int tourn(Poke* joueur1[2], Poke* joueur2[2], int tour, Actif* actif) { // si pokemon ko retourne 1 et met fin a la boucle infini
    int ko;
...
int combat(Poke* joueur1[2], Poke* joueur2[2], Actif* actif) {
    int fin=0;
    int tour=1;
    while(fin!=1) { 
...
int main() {
    ...
    Poke* joueur1[2];
        for(int i=0; i<2 ; i++) {
        joueur1[i]=malloc(sizeof(Poke));
    }
    joueur1[0]->name="Evoli     "; 
    joueur1[0]->type=0;  
    joueur1[0]->pvmax=110; 
    joueur1[0]->pvactuel=joueur1[0]->pvmax;
    joueur1[0]->vit=rand()%101+80; 
    joueur1[0]->attmax=(rand() % 3 + 10) / 10.0;
    joueur1[0]->attactuel=joueur1[0]->attmax; 
    joueur1[0]->agi=rand()%6; 
    joueur1[0]->defmax=(rand() % 3 + 10) / 10.0;
    joueur1[0]->defactuel=joueur1[0]->defmax; 
    char* graphevoli[13]= {"        \\ '.           .' /","         \\  \\ .---. .-' /","          '. '     `\\_.'","            |(),()  |     ,","            (  __   /   .' \\","           .''.___.'--,/\\_,|","          {  /     \\   }   |","           '.\\     /_.'    /","            |'-.-',  `; _.'","            |  |  |   |`","            `\"\"`\"\"`\"\"\"`","",""}; //rep graphique du pokemon
    for(int i=0 ; i<13 ; i++) {
         joueur1[0]->repgraph[i]=graphevoli[i]; 
    }
    joueur1[0]->description="Pokemon de type normal \nUn Pokémon rare qui change d'apparence et d'aptitudes \nau moment d'évoluer en fonction du milieu hostile auquel il veut s'adapter.\n"; // breve description
    for(int i=0; i<4 ; i++) {
        joueur1[0]->attaque[i] = malloc(sizeof(Attaque));
    }
    joueur1[0]->attaque[0]->nom="mimi-queue";
    joueur1[0]->attaque[0]->degat=0.8;
    joueur1[0]->attaque[0]->reussite=80;
    joueur1[0]->attaque[0]->fonction=5;

    joueur1[0]->attaque[1]->nom="charge";
    joueur1[0]->attaque[1]->degat=25;
    joueur1[0]->attaque[1]->reussite=90;
    joueur1[0]->attaque[1]->fonction=0;

    joueur1[0]->attaque[2]->nom="vive-attaque";
    joueur1[0]->attaque[2]->degat=30;
    joueur1[0]->attaque[2]->reussite=80;
    joueur1[0]->attaque[2]->fonction=0;

    joueur1[0]->attaque[3]->nom="bélier";
    joueur1[0]->attaque[3]->degat=35;
    joueur1[0]->attaque[3]->reussite=70;
    joueur1[0]->attaque[3]->fonction=0;




    joueur1[1]->name="Bulbizarre"; 
    joueur1[1]->type=5;  
    joueur1[1]->pvmax=110; 
    joueur1[1]->pvactuel=joueur1[1]->pvmax;
    joueur1[1]->vit=rand()%101+80; 
    joueur1[1]->attmax=(rand() % 3 + 10) / 10.0;
    joueur1[1]->attactuel=joueur1[1]->attmax; 
    joueur1[1]->agi=rand()%6; 
    joueur1[1]->defmax=(rand() % 3 + 10) / 10.0;
    joueur1[1]->defactuel=joueur1[1]->defmax; 
    char* graphbulbizarre[13]= {"               __","          ____-  -____","         /            \\","        /\\____________/\\","       /  /´¯\\    /¯`\\  \\","       \\  \\ º/    \\º / /","        \\ .·´¯`v´¯`·. /","       / \\¨\\______/¨ /  \\","      |  |¯¯¯¯¯¯¯¯¯¯¯|  |","      IooI           IooI","","",""};
    for(int i=0 ; i<13 ; i++) {
         joueur1[1]->repgraph[i]=graphbulbizarre[i]; 
    }
    joueur1[0]->description="Pokemon de type plante \nBulbizarre passe son temps à faire la sieste sous le \nsoleil. Il y a une graine sur son dos. Il absorbe les rayons du soleil pour faire \ndoucement pousser la graine.\n"; // breve description
    for(int i=0; i<4 ; i++) {
        joueur1[1]->attaque[i] = malloc(sizeof(Attaque));
    }
    joueur1[1]->attaque[0]->nom="mimi-queue";
    joueur1[1]->attaque[0]->degat=0.8;
    joueur1[1]->attaque[0]->reussite=80;
    joueur1[1]->attaque[0]->fonction=5;

    joueur1[1]->attaque[1]->nom="fouet";
    joueur1[1]->attaque[1]->degat=25;
    joueur1[1]->attaque[1]->reussite=90;
    joueur1[1]->attaque[1]->fonction=0;

    joueur1[1]->attaque[2]->nom="lance feuille";
    joueur1[1]->attaque[2]->degat=30;
    joueur1[1]->attaque[2]->reussite=80;
    joueur1[1]->attaque[2]->fonction=0;

    joueur1[1]->attaque[3]->nom="photosynthese";
    joueur1[1]->attaque[3]->degat=40;
    joueur1[1]->attaque[3]->reussite=70;
    joueur1[1]->attaque[3]->fonction=1;
    ...
    Poke* joueur2[2];
    for(int i=0; i<2 ; i++) {
        joueur2[i]=malloc(sizeof(Poke));
    }
    joueur2[0]->name="Miaouss   "; // structure pokemon miaouss
    joueur2[0]->type=0;
    joueur2[0]->pvmax=100;
    joueur2[0]->pvactuel=joueur2[0]->pvmax;
    joueur2[0]->vit=rand()%101+80;
    joueur2[0]->attmax=(rand() % 3 + 10) / 10.0;
    joueur2[0]->attactuel=joueur2[0]->attmax;
    joueur2[0]->agi=rand()%6;
    joueur2[0]->defmax=(rand() % 3 + 10) / 10.0;
    joueur2[0]->defactuel=joueur2[0]->defmax;
    char* graphmiaouss[13]= {"               .-. \\_/ .-. ","               \\.-\\/=\\/.-/","            '-./___|=|___\\.-'","           .--| \\|/`\"`\\|/ |--.","          (((_)\\  .---.  /(_)))","           `\\ \\_`-.   .-'_/ /`_","             '.__       __.'(_))", "                 /     \\     //","                |       |__.'/","                \\       /--'`","            .--,-' .--. '----.","           '----`--'  '--`----'",""};
    for(int i=0 ; i<13 ; i++) {
         joueur2[0]->repgraph[i]=graphmiaouss[i];
    }
    joueur2[0]->description="Pokemon de type normal \nMiaouss peut rentrer ses griffes dans ses pattes pour \nrôder gracieusement sans laisser de traces. Étrangement, ce Pokémon raffole\ndes pièces d'or qui brillent à la lumière.\n";
    for(int i=0; i<4 ; i++) {
        joueur2[0]->attaque[i] = malloc(sizeof(Attaque));
    }
    joueur2[0]->attaque[0]->nom="soin";
    joueur2[0]->attaque[0]->degat=30;
    joueur2[0]->attaque[0]->reussite=80;
    joueur2[0]->attaque[0]->fonction=1;

    joueur2[0]->attaque[1]->nom="charge";
    joueur2[0]->attaque[1]->degat=25;
    joueur2[0]->attaque[1]->reussite=90;
    joueur2[0]->attaque[1]->fonction=0;

    joueur2[0]->attaque[2]->nom="damocles";
    joueur2[0]->attaque[2]->degat=1.2;
    joueur2[0]->attaque[2]->reussite=80;
    joueur2[0]->attaque[2]->fonction=3;

    joueur2[0]->attaque[3]->nom="plaquage";
    joueur2[0]->attaque[3]->degat=35;
    joueur2[0]->attaque[3]->reussite=70;
    joueur2[0]->attaque[3]->fonction=0;


    joueur2[1]->name="Chartor   "; // structure pokemon miaouss
    joueur2[1]->type=3;
    joueur2[1]->pvmax=100;
    joueur2[1]->pvactuel=joueur2[1]->pvmax;
    joueur2[1]->vit=rand()%101+80;
    joueur2[1]->attmax=(rand() % 3 + 10) / 10.0;
    joueur2[1]->attactuel=joueur2[1]->attmax;
    joueur2[1]->agi=rand()%6;
    joueur2[1]->defmax=(rand() % 3 + 10) / 10.0;
    joueur2[1]->defactuel=joueur2[1]->defmax;
    char* graphchartor[13]= {"                  __","       .,-;-;-,. /'_\\","     _/_/_/_|_\\_) /","   '-<_><_><_><_>=/\\","     `/_/====/_/-'\\_\\","      \"\"     \"\"    \"\"","","","","","","",""};
    for(int i=0 ; i<13 ; i++) {
         joueur2[1]->repgraph[i]=graphchartor[i];
    }
    joueur2[1]->description="Pokemmon de type feu \nUn bon indicateur de sa santé est le débit de la fumée qui s'échappe de sa \ncarapace: s'il est important, c'est qu'il est en pleine forme.\n";
    for(int i=0; i<4 ; i++) {
        joueur2[1]->attaque[i] = malloc(sizeof(Attaque));
    }
    joueur2[1]->attaque[0]->nom="lance flamme";
    joueur2[1]->attaque[0]->degat=30;
    joueur2[1]->attaque[0]->reussite=80;
    joueur2[1]->attaque[0]->fonction=1;

    joueur2[1]->attaque[1]->nom="flameche";
    joueur2[1]->attaque[1]->degat=25;
    joueur2[1]->attaque[1]->reussite=90;
    joueur2[1]->attaque[1]->fonction=0;

    joueur2[1]->attaque[2]->nom="damocles";
    joueur2[1]->attaque[2]->degat=1.2;
    joueur2[1]->attaque[2]->reussite=80;
    joueur2[1]->attaque[2]->fonction=3;

    joueur2[1]->attaque[3]->nom="boule de feu";
    joueur2[1]->attaque[3]->degat=35;
    joueur2[1]->attaque[3]->reussite=70;
    joueur2[1]->attaque[3]->fonction=0;
