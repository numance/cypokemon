...
int afficherMenu(char* menu[], int tailleMenu, int numpoke, Poke* Pokemon[18]) { 
    int choix = 0;  
    int touche;
...
int main() { //fonction principale
    ...
    for(int i=0; i<3 ; i++) {
        choix = afficherMenu(menu, 18, i+1, Pokemon); // choix du pokemon du j1
        joueur1[i]=Pokemon[choix]; // pokemon choisi inserer dans l'equipe
    }
    for(int i=0; i<3 ; i++) {
        choix = afficherMenu(menu, 18, i+4, Pokemon); //meme pour j2
        joueur2[i]=Pokemon[choix]; // meme pour j2
    }
    system("clear");
    return 0;
}